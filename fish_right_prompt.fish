...
# => Toggle functions
#####################
function __mudfencer_toggle_symbols -d 'Toggles style of symbols, press # in NORMAL or VISUAL mode'
...
function __mudfencer_toggle_pwd -d 'Toggles style of pwd segment, press space bar in NORMAL or VISUAL mode'
  for i in (seq (count $mudfencer_pwdstyle))
    if [ $mudfencer_pwdstyle[$i] = $pwd_style ]
      set pwd_style $mudfencer_pwdstyle[(expr $i \% (count $mudfencer_pwdstyle) + 1)]
      set pwd_hist_lock true
      commandline -f repaint
      break
      end
...
# => Command duration segment
#############################
function __mudfencer_cmd_duration -d 'Displays the elapsed time of last command'
  set -l seconds ''
  set -l minutes ''
  set -l hours ''
  set -l days ''
  ...
  if [ $cmd_duration -gt 0 ]
    ...
    if [ $cmd_duration -ge 60 ]
      ...
      set_color $mudfencer_colors[2]
      echo -n ''
      switch $pwd_style
        case short long
          if [ $last_status -ne 0 ]
            echo -n (set_color -b $mudfencer_colors[2] $mudfencer_colors[7])' '$days$hours$minutes$seconds' '
          else
            echo -n (set_color -b $mudfencer_colors[2] $mudfencer_colors[12])' '$days$hours$minutes$seconds' '
          end
      end
    set_color -b $mudfencer_colors[2]
  end
end
...
# => Git segment
################
function __mudfencer_is_git_ahead_or_behind -d 'Check if there are unpulled or unpushed commits'
...
function __mudfencer_git_status -d 'Check git status'
...
function __mudfencer_is_git_stashed -d 'Check if there are stashed commits'
  command git log --format="%gd" -g $argv 'refs/stash' -- 2> /dev/null | wc -l | tr -d '[:space:]'
end

function __mudfencer_prompt_git_symbols -d 'Displays the git symbols'
  set -l is_repo (command git rev-parse --is-inside-work-tree 2> /dev/null)
  if [ -z $is_repo ]
    return
  end

  set -l git_ahead_behind (__mudfencer_is_git_ahead_or_behind)
  set -l git_status (__mudfencer_git_status)
  set -l git_stashed (__mudfencer_is_git_stashed)
      ...
      case long short
        if [ $symbols_style = 'symbols' ]
          if [ $git_ahead_behind[1] -gt 0 ]
            set_color -o $mudfencer_colors[5]
            echo -n ' ↑'
          end
          ...
          if [ $git_status[1] -gt 0 ]
            set_color -o $mudfencer_colors[12]
            echo -n ' +'
          end
          if [ $git_status[2] -gt 0 ]
            set_color -o $mudfencer_colors[7]
            echo -n ' –'
          end
          if [ $git_status[3] -gt 0 ]
            set_color -o $mudfencer_colors[10]
            echo -n ' ✱'
          end
          if [ $git_status[4] -gt 0 ]
            set_color -o $mudfencer_colors[8]
            echo -n ' →'
          end
          if [ $git_status[5] -gt 0 ]
            set_color -o $mudfencer_colors[9]
            echo -n ' ═'
          end
          if [ $git_status[6] -gt 0 ]
            set_color -o $mudfencer_colors[4]
            echo -n ' ●'
          end
          if [ $git_stashed -gt 0 ]
            set_color -o $mudfencer_colors[11]
            echo -n ' ✭'
          end
        else
          if [ $git_ahead_behind[1] -gt 0 ]
            set_color $mudfencer_colors[5]
          ...
          if [ $git_status[1] -gt 0 ]
            set_color $mudfencer_colors[12]
            echo -n ' '$git_status[1]
          end
          if [ $git_status[2] -gt 0 ]
            set_color $mudfencer_colors[7]
            echo -n ' '$git_status[2]
          end
          if [ $git_status[3] -gt 0 ]
        ...
        set_color -b $mudfencer_colors[3] normal
        echo -n ' '
    end
  end
end
...
# => PWD segment
################
function __mudfencer_prompt_pwd -d 'Displays the present working directory'
  set -l user_host ' '
  if set -q SSH_CLIENT
    if [ $symbols_style = 'symbols' ]
      switch $pwd_style
  ...
  set_color $mudfencer_current_bindmode_color
  echo -n ''
  set_color normal
  set_color -b $mudfencer_current_bindmode_color $mudfencer_colors[1]
  if [ (count $mudfencer_prompt_error) != 1 ]
    switch $pwd_style
      case short
...
function fish_right_prompt -d 'Write out the right prompt of the mudfencer theme'
  echo -n -s (__mudfencer_cmd_duration) (__mudfencer_prompt_git_symbols) (__mudfencer_prompt_pwd)
  set_color normal
end
